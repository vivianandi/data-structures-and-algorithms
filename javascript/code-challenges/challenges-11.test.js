...
const longestString = (arr) => {
  ...
  for (let i = 1; i < arr.length; i++) {
    if (arr[i].substr(0, arr[i].length).length > arr[longestIndex].substr(0, arr[longestIndex].length).length) {
      longestIndex = i;
    }
  }
  return longestIndex;
...
const firstLetters = (arr) => {
  ...
  for (let i = 0; i < arr.length; i++) {
    firstLettersArray.push(arr[i].substr(0, 1));
  }
...
const findHappiness = (arr) => {
  const happyStrings = [];
  for (let i = 0; i < arr.length; i++) {
    if (arr[i].substr(0, arr[i].length).includes(":)")) {
      happyStrings.push(arr[i]);
    }
  }
  return happyStrings;
};
...
const standardizePhoneNumbers = (arr) => {
  const standardizedNumbers = [];
  for (let i = 0; i < arr.length; i++) {
    const standardizedNumber = arr[i].replace(/[() -]/g, '');
    standardizedNumbers.push(standardizedNumber);
  }
